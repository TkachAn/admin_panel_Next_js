...
export async function GET(request) {
  ...
  try {
    ...
    if (rows.length > 0) {
      return NextResponse.json({ exists: true, owner: rows[0] });
    } else {
      return NextResponse.json({ exists: false });
    }
  } catch (err) {
    console.error('Ошибка при поиске владельца:', err);
    return NextResponse.json({ message: 'Ошибка сервера' }, { status: 500 });
  } finally {
    if (connection) connection.release();
  }
...
export async function POST(request) {
  ...
  try {
    ...
    return NextResponse.json({ owner: { id: result.insertId, owner_name } });
  } catch (err) {
    console.error('Ошибка при создании владельца:', err);
    return NextResponse.json({ message: 'Ошибка сервера' }, { status: 500 });
  } finally {
